...
local PRODUCT_KEY = "a1WGqFrVS2z"
local DEVICE_NAME = "862167058169602"
local DEVICE_SECRET = "65155321cfeed88476a744bfc4eb8bdd"
...
function publishTest()
    if sConnected then
        ...
        aLiYun.publish("/"..PRODUCT_KEY.."/"..getDeviceName().."/update", "payload安慰肥牛发微风啊我", 0)
    end
end
...
local function connectCbFnc(result)
    ...
    if result then
        ...
        --aLiYun.subscribe({["/"..PRODUCT_KEY.."/"..getDeviceName().."/user/subscribe"]=0, ["/"..PRODUCT_KEY.."/"..getDeviceName().."/user/subscribe"]=1})
        --注册数据接收的处理函数
...
local function read()
    ...
    while true do
        ...
        --打开下面的打印会耗时
        print(data)
	aLiYun.publish("/"..PRODUCT_KEY.."/"..getDeviceName().."/user/publish", data, 0)
